...
namespace Ironfield.EditorTools
{
    ...
    /// Renders on URP (migrated 2026-09-12 — see EnsureUrpAsset/EnsureUrpVolume
    /// and MakeStandard/MakeUnlit/MakeFx for the Built-in-&gt;URP shader swap).
    /// </summary>
    public static class IronfieldSetup
    {
        ...
        /// <summary>Creates (or finds) the project's URP pipeline asset + renderer
        /// data and assigns it as the active pipeline in both GraphicsSettings
        /// (the "always use this" default) and QualitySettings (the active
        /// quality level's override) — Unity checks both, and headless batchmode
        /// has no Inspector to click through the migration wizard, so this has to
        /// be done by script instead of Assets/Create/Rendering/URP Asset.</summary>
        [MenuItem("Ironfield/0b. Ensure URP Pipeline Asset")]
        public static void EnsureUrpAsset()
        {
            const string rendererPath = SettingsDir + "/IronfieldRendererData.asset";
            const string assetPath = SettingsDir + "/IronfieldURP.asset";
            ...
            // shadow tuning replaces the old QualitySettings.shadowDistance/
            // shadowCascades/shadowResolution calls (URP owns shadow config on
            // the pipeline asset, not QualitySettings, once it's the active RP).
            pipelineAsset.shadowDistance = 420f;
            pipelineAsset.shadowCascadeCount = 4;
            pipelineAsset.cascade4Split = new Vector3(0.067f, 0.2f, 0.467f);
            // soft shadows are a read-only capability flag on the asset, driven
            // by softShadowQuality/per-light Shadows setting instead — the sun
            // Light below is already set to LightShadows.Soft.
            pipelineAsset.shadowNormalBias = 0.5f;
            ...
            AssetDatabase.SaveAssets();
            Debug.Log("[Ironfield] URP pipeline asset ensured and assigned as active RP.");
        }
        ...
        public static void Run()
        {
            ...
            // ALL materials, not just M_ext_* — every one of them is generated by
            // MakeStandard/MakeUnlit/MakeFx/etc, so a shader/property change (like
            // the Built-in->URP migration) needs a clean regenerate to actually
            // take effect on disk, not just in new code paths.
        ...
        static void BuildScene(MissionBuildConfig cfg)
        {
            ...
            // vestigial under URP (shadow config now lives on the pipeline asset,
            // see EnsureUrpAsset) but harmless to leave set for Built-in fallback.
            ...
            // URP post-processing (replaces the old Built-in OnRenderImage
            // CameraPost/IronfieldPost.shader — URP's renderer never calls
            // OnRenderImage, so that component was dead code once the pipeline
            // asset switched over): a Global Volume drives the grade, the camera
            // just needs to opt in.
            ...
            vol.isGlobal = true;
            vol.priority = 0f;
            vol.profile = BuildPostVolumeProfile();
        ...
        static void BuildRoadMesh(Terrain terrain, List<Transform> waypoints)
        {
            ...
            var roadMat = new Material(Shader.Find(LitShader)) { name = "RoadRibbon" };
            ...
            roadMat.SetFloat("_Smoothness", 0.05f);
        ...
        // small helpers
        // ----------------------------------------------------------------- //
        /// <summary>URP Volume profile approximating the old CameraPost/
        /// IronfieldPost.shader grade (punchy contrast/saturation + a
        /// vignette) via the built-in ColorAdjustments/Vignette overrides that
        /// ship with URP's core Volume framework — no post-processing package
        /// needed, URP 14+ bundles this. Post Exposure is EV (0 = neutral,
        /// matching the old code's exposure=1.0/no-op); contrast/saturation are
        /// -100..100 percent, close enough to the old +11%/+14% multipliers to
        /// read the same. Not a pixel-exact port of the old shader — the shadow
        /// lift tint was dropped as a minor, hard-to-verify-headless detail.</summary>
        static VolumeProfile BuildPostVolumeProfile()
        {
            const string p = SettingsDir + "/PostVolumeProfile.asset";
            var existing = AssetDatabase.LoadAssetAtPath<VolumeProfile>(p);
            if (existing != null) return existing;

            var profile = ScriptableObject.CreateInstance<VolumeProfile>();
            var color = profile.Add<ColorAdjustments>(true);
            color.postExposure.overrideState = true; color.postExposure.value = 0f;
            color.contrast.overrideState = true; color.contrast.value = 11f;
            ...
            AssetDatabase.CreateAsset(profile, p);
            return profile;
        }
        ...
        static Material MakeGroundMaterial()
        {
            ...
            if (existing != null) return existing;
            var m = new Material(Shader.Find(LitShader)) { name = "Ground" };
        ...
        static Material MakeUnlit(Color c, string name)
        {
            ...
            if (existing == null)
            {
                existing = new Material(Shader.Find(LitShader)) { name = name, color = c };
                existing.SetFloat("_Smoothness", 0.06f);
        ...
        /// <summary>Genuinely unlit, alpha-blended FX material (tracers, particle quads).</summary>
        static Material MakeFx(Color c, string name)
        {
            ...
            if (existing == null)
            {
                var shader = Shader.Find(UnlitShader) ?? Shader.Find(LitShader);
                existing = new Material(shader) { name = "fx_" + name, color = c };
                SetTransparent(existing);
                AssetDatabase.CreateAsset(existing, p);
            }
        ...
        /// <summary>Flips a URP Lit/Unlit material to the Transparent surface type —
        /// the same property/keyword/queue combination the shader's own Inspector
        /// GUI sets when you switch Surface Type to Transparent there. Used for FX
        /// (particles, tracers) that need alpha blending and no depth write, the
        /// same role Built-in's "Sprites/Default" used to play.</summary>
