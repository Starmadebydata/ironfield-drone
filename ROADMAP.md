...
## 阶段外补充:无人机低飞植物悬空(2026-09-13,用户实测反馈)

用户原话:"无人机在低飞时，发现很多植物是悬空的"。

根因排查:`ScatterVegetation` 给每棵树/灌木套用的是**非等比** Y 缩放
(`localScale = (s, s*(0.85~1.25), s)`,专门用来做高矮变化),但外部模型的
"贴地对齐"是 `LoadExternalModel` 在**固定的等比缩放** `k` 下一次性算好、
烘焙进预制体的——这个对齐只在"实例化时缩放比例和烘焙时一致"的前提下才
严格成立(等比缩放时数学上能精确抵消;一旦 Y 轴单独缩放,而模型自身又带有
导入时的轴向修正旋转把局部 Y 和 X/Z 混在一起,基座就会跟着偏离地面)。修复:
不再依赖"烘焙对齐能扛住任意后续缩放"这个假设,改成每棵树/灌木在设好最终
缩放和朝向之后,直接测量一次真实渲染包围盒,把残差直接贴回地面高度——这个
做法不依赖具体是哪种轴向/旋转导致的偏移,天然兜底。同样的问题也顺手修了
路边树篱(hedgerow)灌木的贴地。EditMode 21/21,PlayMode 26/26,已通过完整
headless 重建 + Release build 验证(0 错误 0 警告);肉眼确认因本 session
后段的实机交互工具故障(见下条)未能完成,待用户本人试玩确认贴地效果。

## 阶段外补充:全屏无法退出 + 植物模型粗糙(2026-09-13,用户直接提出)

用户原话:"游戏现在好像只有全屏模式，此外很多植物模型还非常粗糙，需要继续
细化构建"。

1. ✅ **全屏问题**:项目此前完全没有暴露任何窗口/全屏切换——`ProjectSettings`
   里 `fullscreenMode: 1`(FullScreenWindow,无边框全屏)是唯一状态,只能靠
   系统级 Alt+Enter 切出(绝大多数玩家不知道有这个快捷键)。新增
   `GameSettings.Fullscreen`(PlayerPrefs 持久化,默认 true 维持原行为)+
   `SettingsGUI` 里的"全屏"开关,调用 `Screen.fullScreenMode` /
   `Screen.SetResolution` 切换;同时把 `ProjectSettings.asset` 的
   `resizableWindow` 从 0 改成 1,窗口模式下可以拖拽调整大小。
2. ✅ **植物模型粗糙**:排查后确认问题集中在 Bush(灌木)——这是植被里
   唯一还在用程序化占位模型的品种(3 个叠加的球体),其余阔叶/针叶/枯树
   早就换成了真实外部 CC0/CC-BY 模型(见更早的"内容密度"/初期外部素材
   记录)。从 poly.pizza 找到同一位作者(**Quaternius**,已经用在阔叶树上
   的 CC0 作者)的灌木模型换上("Bush",poly.pizza/m/ooG6CkLyE8,CC0,已
   登记进 CREDITS.md)。
   **过程中顺带修了两个真实问题**:
   - Poly Pizza 下载包里常带一个来自展示缩略图场景的多余 "Cube" 网格
     (背景参考板),原样转换进 fbx 后会作为一个真实可见的方块混进模型——
     `LoadExternalModel` 现在会无条件剔除任何名字精确等于 "Cube" 的子物体
     (没有任何正经模型部件会用这个名字,剔除是安全的)。
   - 灌木模型自带一张真实叶片贴图(`Leaf_Texture.png`),但 Unity 对这个
     fbx 的默认导入设置(`materialImportMode: External`)不会把内嵌贴图
     带进来——之前项目里所有外部模型统一靠 `ApplyPalette`/`palettes.txt`
     从 .glb 提取"纯色"重建材质,本来就没打算依赖 FBX 自带材质,这次是
     第一次真的需要保留一张贴图而不是纯色。改用 Blender 直接从源 .glb
     里把贴图另存成 `bush_leaf.png`,新增 `MakeTexturedStandard` 辅助方法
     显式挂载,`ApplyPalette` 对 `modelName=="bush"` 单独走这条路径,不
     影响其余模型现有的纯色流程。
3. ⚠️ **实机验证受阻,未完成**:本轮修改已经过完整 headless 重建 +
   EditMode 21/21 / PlayMode 26/26 全绿 + Release build(0 错误 0 警告)
   验证,但尝试用 `computer-use` 截图/点击实际验证全屏开关和灌木贴图效果
   时,工具反复报告"点击位置属于通知中心"且无法通过常规方式解除(疑似这个
   未签名/未公证的 macOS 构建触发了某种辅助功能识别异常,和 P3 阶段记录过
   的 Gatekeeper 隔离问题类似,但表现不同)。这是本 session 第二次撞上
